...
class KitchenTestCase(BaseTestClass):
    ...
    def test_kitchen_menu(self):
        res = self.client.post("/api/v1.0/kitchen/menu", json={"name": "Chapina Chorizo", "description": "Doble Torta de carne Premium Blend, Queso Americano, Chorizo asado, Salsa Chicharronera, Tomate, Cebolla y Fresca Lechuga. Incluye Curly Fries y Frezka Natural.", "price": "64.00"})
    ...
    def test_kitchen_extra_category_create(self):
        res = self.client.post("/api/v1.0/kitchen/extras-category", json={"name": "Bebidas"})
        self.assertEqual("Extra category created successfully", res.json["message"])
        self.assertEqual(201, res.json["status_code"])
    ...
    def test_kitchen_extra_create(self):
        ...
        with self.app.app_context():
            ...
            res = self.client.post("/api/v1.0/kitchen/extras", json={"name": "Bebidas de naranja", "price": 12.5, "extras_category_id": ec.id})
            self.assertEqual(201, res.json["status_code"])
    ...
    def test_kitchen_order_create(self):
        ...
        # with self.app.app_context():
        #     # - create rol
        #     rol = Rol(name="customer")
        #     rol.save()
        #     # - create an user
        #     user_data = dict(username="Kevin", email="kevin@gmail.com", password="test", rol_id=rol.id)
        #     user = Users(**user_data)
        #     user.save()
        #     user = Users.get_by_email("kevin@gmail.com")
            
        

        # get orders_menu (items menu)
        # verify orders-extras
        # create order